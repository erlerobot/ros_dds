...
// This code implements a rosnode list prototype 
// using bare calls of OpenSplice DDS. The code is 
// based on Discovery.cpp
...
int OSPL_MAIN (int argc, char *argv[]) {
    ...
    std::cout << "rosnode list prototype using bare DDS calls:" << std::endl;
    std::cout << "-------------------------------------------:" << std::endl;
    ...
    // create_participant(in DomainId_t domainId, in DomainParticipantQos qos, 
    //                      in DomainParticipantListener a_listener, in StatusMask mask);
        ...
        for(int i = 0; i < infoSeq->length(); ++i) {
            ...
            switch(info->instance_state) {
                case ALIVE_INSTANCE_STATE:
                    std::cout << "DDS participant joined" << std::endl;
